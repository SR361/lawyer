...
class VamtamElementorBridge {
	...
	public static function add_sticky_header_bg_color_to_elementor_global_settings() {
		...
		$kit->add_repeater_row( 'system_colors', [
			...
			'title' => __( 'Sticky Header Bg Color', 'avvocato' ),
	...
	*/
		...
		if ( $use_el_global_defaults ) {
			...
			for ( $i = 1; $i <= 8; $i++ ) {
				...
				$kit->add_repeater_row( 'system_colors', [
					...
					'title' => sprintf( __( 'Accent %s', 'avvocato' ), $i ),
	...
	protected static function add_theme_fonts_to_elementor_global_fonts() {
		...
		if ( $use_el_global_defaults ) {
			...
			// Adding from vamtam_get_option().
			$our_fonts = [
				'primary-font' => __( 'Primary Font', 'avvocato' ),
				'h1'           => __( 'H1', 'avvocato' ),
				'h2'           => __( 'H2', 'avvocato' ),
				'h3'           => __( 'H3', 'avvocato' ),
				'h4'           => __( 'H4', 'avvocato' ),
			...
			foreach ( $our_fonts as $font_prefix => $font_title ) {
				...
				$kit->add_repeater_row( 'system_colors', [
					...
					'title' => sprintf( __( '%s', 'avvocato' ), $font_title ),
	...
	public static function register_locations( $elementor_theme_manager ) {
		...
		$elementor_theme_manager->register_location(
			'page-title-location',
			[
				'label' => esc_html__( 'Page Title', 'avvocato' ),
				'multiple' => true,
	...
	public static function get_widget_mods_list() {
		...
		*/
		...
		$widget_mods_list = [
			...
			'button'                                     => [ 'label' => __( 'Button', 'avvocato' ) ],
			'form'                                       => [ 'label' => __( 'Form', 'avvocato' ) ],
			'tabs'                                       => [ 'label' => __( 'Tabs', 'avvocato' ) ],
			'image'                                      => [ 'label' => __( 'Image', 'avvocato' ) ],
			'nav-menu'                                   => [ 'label' => __( 'Nav Menu', 'avvocato' ) ],
			'search-form'                                => [ 'label' => __( 'Search Form', 'avvocato' ) ],
			'section'                                    => [ 'label' => __( 'Section', 'avvocato' ) ],
			'toggle'                                     => [ 'label' => __( 'Toggle', 'avvocato' ) ],
			'archive-posts'                              => [ 'label' => __( 'Archive Posts', 'avvocato' ) ],
			'posts'                                      => [ 'label' => __( 'Posts', 'avvocato' ) ],
			'post-comments'                              => [ 'label' => __( 'Post Comments', 'avvocato' ) ],
			'popup' /* Not a widget but same logic */    => [ 'label' => __( 'Popup', 'avvocato' ) ],
		];
	...
	*/
	...
	public static function extract_zip( $file, $to ) {
		...
		if ( ! $unzip_result ) {
			$unzip_result = new \WP_Error( 'error', esc_html__( 'Could not unzip or empty archive.', 'avvocato' ) );
		}
