...
function vamtam_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			'name'     => esc_html__( 'Contact Form by WPForms', 'avvocato' ),
			'slug'     => 'wpforms-lite',
			'required' => false,
		...
		array(
			'name'     => esc_html__( 'Elementor', 'avvocato' ),
			'slug'     => 'elementor',
			'required' => true,
			'category' => 'required',
			'version'  => '2.7.4',
		...
		array(
			'name'     => esc_html( sprintf( __( 'Vamtam Elementor Integration (%s)', 'avvocato' ), VAMTAM_THEME_SLUG ) ),
		...
		array(
			'name'     => esc_html__( 'Vamtam Importers (E)', 'avvocato' ),
		...
		array(
			'name'     => esc_html__( 'Wordpress SEO', 'avvocato' ),
			'slug'     => 'wordpress-seo',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'Really Simple SSL', 'avvocato' ),
		...
		array(
			'name'     => esc_html__( 'Loco Translate', 'avvocato' ),
			'slug'     => 'loco-translate',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'UpDraftPlus', 'avvocato' ),
			'slug'     => 'updraftplus',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'WP Super Cache', 'avvocato' ),
			'slug'     => 'wp-super-cache',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'Limit Login Attempts Reloaded', 'avvocato' ),
		...
		array(
			'name'     => esc_html__( 'Wordfence', 'avvocato' ),
			'slug'     => 'wordfence',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'Self-hosted Google Fonts', 'avvocato' ),
		...
		array(
			'name'     => esc_html__( 'CookieYes | GDPR Cookie Consent & Compliance Notice (CCPA Ready)', 'avvocato' ),
...
function vamtam_tgmpa_table_columns( $columns ) {
	...
	$columns = array(
		...
		'img'         => '',
		'plugin'      => esc_html__( 'Name', 'avvocato' ),
		'description' => esc_html__( 'Description', 'avvocato' ),
		'status'      => esc_html__( 'Status', 'avvocato' ),
		'version'     => esc_html__( 'Version', 'avvocato' ),
	);
	return $columns;
}
...
function vamtam_tgmpa_table_data_item( $item, $plugin )
{
	...
	if( isset( $plugin_data['Description'] ) ) {
		...
		$item['description'] =  esc_html__( 'A simple WordPress Plugin.', 'avvocato' );
	}
	if ( strpos( $plugin['slug'], 'avvocato' ) !== false ) {
		$item['description'] =  esc_html__( 'Theme Exclusive.', 'avvocato' );
	}
...
function vamtam_tgmpa_admin_menu_args( $args ) {
	$args['page_title'] = esc_html__( 'Install Plugins', 'avvocato' );
